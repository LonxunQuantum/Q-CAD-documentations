...
.. note::
 1. SYS_TYPE: specifies type of the system and automatically adjust the parameter "FERMIDE", 1: semiconductor or insulator (default, FERMIDE=0.025); 2: metallic (FERMIDE=0.2).
...
2.Abtain the final.config file, which contains the position of the last ionic step of the relaxation, and can be used for step two "SCF" calculation.

Download
---------

`Step1_initial_relax_cal <examples/Step1_initial_relax_cal.tar.gz>`_
...
`Step1_final_relax_cal <examples/Step1_final_relax_cal.tar.gz>`_
...
.. note::
   1. Copy final.config from previous initial state RELAX calculation, and rename atom.config
...
.. note::
   1. Copy final.config from previous final state RELAX calculation, and rename atom.config
...
.. note::
   ...
   | 4.FORCE_TOL: the atomic force tolerance (eV/Å) to stop the relaxation.
   | 5.NIMAGE: the number of images in the NEB.
   | 6.AK: the spring constant for the image string (eV/ :math:`Å^{2}`) .
...
.. note::
   ...
   | 3.angle(cos(th))(fourth column): the :math:`cos(\theta`) of the angle theta between two R(image + 1) − R(image), and R(image) − R(image − 1). 

Download
---------

`Step3_neb_cal <examples/Step3_neb_cal.tar.gz>`_
